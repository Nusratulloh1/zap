...
/*
  Поверхность кнопок шапки главной: чернила экрана плюс белые 8%. Нужна как
  запасной цвет — при «Уменьшении прозрачности» iOS снимает блюр совсем.
*/
  ...
  */
  ...
  return (
    <View pointerEvents="box-none" style={[styles.wrap, { paddingBottom: Math.min(insets.bottom, 20) + 10 }]}>
      ...
      */}
      {/*
        В тёмной теме пилл — стекло на всю площадь: поверх блюра только белые
        8%, тот же слой, что и у кнопок шапки главной. Плотная заливка тон
        совпадала точно, но убивала сам блюр — стекло оставалось лишь по краям.
        Плотный цвет держим как fallback: он подставляется, когда в системе
        включено «Уменьшение прозрачности».
      */}
      <Glass
        thin
        dark={dark}
...
const styles = StyleSheet.create({
  ...
  */
  /*
    ФОНА ЗДЕСЬ БЫТЬ НЕ ДОЛЖНО. Он рисуется ЗА BlurView, и тогда размывается
    он сам, а не экран под пиллом — стекло превращается в плоскую заливку при
    любом материале. Именно это и ломало эффект. Тон уехал в проп tint, он
