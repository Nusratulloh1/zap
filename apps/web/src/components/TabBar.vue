...
// трёхпозиционный свитчер по дизайну: главная / пад суммы / история
const tabs = [
  { path: '/', icon: 'home' },
  { path: '/split/amount', icon: 'grid' },
...
<template>
  <nav class="pointer-events-none fixed inset-x-0 bottom-[calc(14px+env(safe-area-inset-bottom))] z-30 mx-auto flex w-full max-w-app justify-center">
    <!-- стеклянная отрисовка пилла — в .zap-tabbar (main.css): blur + @supports-фолбэк -->
    <div class="zap-tabbar pointer-events-auto flex items-center gap-[26px] rounded-full px-3.5 py-2">
      <button
        v-for="t in tabs"
        :key="t.path"
        type="button"
        class="tab-btn press flex h-[46px] w-[46px] items-center justify-center rounded-full transition-colors duration-200"
        :class="[route.path === t.path && 'is-active', route.path === t.path && (t.icon === 'grid' ? 'bg-[#111110]' : 'bg-lime')]"
        :aria-label="t.path"
        @click="go(t.path, $event)"
      >
        <svg v-if="t.icon === 'home'" width="22" height="22" viewBox="0 0 24 24" fill="none">
          <path d="M4 11.5L12 4.5L20 11.5V19.5H14.5V14.5H9.5V19.5H4V11.5Z" stroke="currentColor" stroke-width="2.2" stroke-linejoin="round" />
        </svg>
        <!-- активный пад: чернильный круг с лаймовыми точками (дизайн 3a) -->
        <div v-else-if="t.icon === 'grid'" class="grid grid-cols-[5px_5px_5px] gap-[5px]">
          <div
            ...
            :style="{ background: route.path === t.path ? '#DDFF33' : 'currentColor' }"
          />
        </div>
        <svg v-else width="23" height="23" viewBox="0 0 24 24" fill="none">
          <circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="2.2" />
          <path d="M12 7.5V12L15 14" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" />
        </svg>
      </button>
    </div>
  </nav>
