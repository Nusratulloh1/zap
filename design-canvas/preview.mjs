// Плоские превью артбордов для визуальной проверки: та же вёрстка, но без
// обёртки Design Components — можно открыть в браузере и снять скриншот.
import { readFileSync, writeFileSync } from 'node:fs'

for (const name of ['Main', 'Landing', 'LandingMobile']) {
  const src = readFileSync(new URL(`./${name}.dc.html`, import.meta.url), 'utf8')
  const helmet = src.split('<helmet>')[1].split('</helmet>')[0]
  const body = src.split('</helmet>')[1].split('</x-dc>')[0]
  writeFileSync(
    new URL(`./preview-${name}.html`, import.meta.url),
